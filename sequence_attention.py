...
# =================================================================
# SECTION 2.1 : SYNTHETIC DATASET (Sine waves with trends)
# =================================================================
...
# =================================================================
# SECTION 1.2 : CUSTOM ATTENTION LAYER (Exercice 1)
# =================================================================
class SimpleAttention(layers.Layer):
    ...
    def build(self, input_shape):
        # W shape: (hidden_dim, 1) | b shape: (seq_len, 1)
        self.W = self.add_weight(name="att_weight", shape=(input_shape[-1], 1), initializer="normal")
        self.b = self.add_weight(name="att_bias", shape=(input_shape[1], 1), initializer="zeros")
        super(SimpleAttention, self).build(input_shape)

    def call(self, x):
        # Score calculation: dot product + tanh (TODO Ligne 20)
        e = tf.tanh(tf.matmul(x, self.W) + self.b)
        # Softmax for alignment weights (TODO Ligne 21)
        alignment_weights = tf.nn.softmax(e, axis=1)
        # Context vector: weighted sum (TODO Ligne 22)
        context_vector = tf.reduce_sum(x * alignment_weights, axis=1)
        return context_vector, alignment_weights

# =================================================================
# SECTION 3 : RESEARCH CHALLENGE (Temporal Latent Space - H-TAP)
# =================================================================
class TemporalTransformerBlock(layers.Layer):
    """Implémentation manuelle de la logique Transformer pour le défi."""
...
# =================================================================
# MODÈLES POUR COMPARAISON (Baseline TAP vs Improved H-TAP)
# =================================================================
def build_baseline_tap(seq_len):
    ...
    # Hybrid RNN + Attention (Exercice 2.1)
...
def build_improved_htap(seq_len):
    ...
    # Amélioration : Bloc Temporel (Partie 3.2)
...
if __name__ == "__main__":
    ...
    for mode in ["Baseline", "Improved"]:
        with mlflow.start_run(run_name=f"Run_{mode}"):
            ...
            # MLOps : Calcul de l'Attention Span (p. 3)
            # On simule l'attention span par l'entropie ou l'écart-type des poids
            mlflow.log_param("architecture", mode)
            mlflow.log_metric("MSE", mse)
            print(f"Modèle {mode} -> MSE: {mse:.4f}")

    # VISUALISATION (Objectif p. 1)
    htap_model, att_layer = build_improved_htap(SEQ_LEN)
    # Extraction des poids d'attention pour un échantillon
    ...
    plt.title("Visualisation des Poids d'Attention (Scaled Dot-Product)")
    plt.xlabel("Time Steps (Séquence d'entrée)")
    plt.ylabel("Poids (Importance)")
    plt.savefig("attention_viz.png")
    print("Visualisation sauvegardée dans 'attention_viz.png'.")